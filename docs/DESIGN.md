...
**Flows.** Export (Settings → Backup): set a passphrase (confirm + "no recovery if forgotten" warning) → encrypt off the UI thread → write the file to a user-chosen location via the OS save-document dialog (`file_picker.saveFile`). Restore (Settings → Backup, and a "Restore from backup" affordance on the onboarding Welcome screen): pick a file (`file_picker.pickFiles`) → reject foreign/too-new/damaged files *before* prompting → enter passphrase (re-prompt on wrong) → confirm summary → apply → the host refreshes appearance providers and re-bootstraps to Home. Implementation: `lib/services/backup/` (codec + service) and `lib/settings/backup_flow.dart` (shared UI).

A real filesystem save (choose a folder) is used rather than a share sheet — `share_plus` can only hand the file to another app, not save it where the user wants. `file_picker` supplies both the save and open dialogs; it pins `win32 ^5.9.0` against `share_plus`'s `^6.0.1`, resolved with a `dependency_overrides: win32: ^6.0.1` (win32 is Windows-only and this app targets Android/iOS, so no shipped binary is affected).
