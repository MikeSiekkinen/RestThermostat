...
///
/// Lives in `services/` (not either screen's feature directory) because both
/// the onboarding and settings modules depend on it.
  ...
  switch (error) {
    case NleAuthError(:final isCloudflareAccess):
      return isCloudflareAccess
          ...
          : l.connectFailedAuth;
    case NleRedirectError(:final target):
      return l.connectFailedRedirect(target.isEmpty ? '?' : target);
